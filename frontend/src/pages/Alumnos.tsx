...
interface AlumnoDraft extends Alumno {
  nombreCompleto?: string;
  notas?: string;
}

const alumnoInicial: AlumnoDraft = {
  nombre: '',
  apellido: '',
  dni: '',
  telefono: '',
...
const Alumnos: React.FC = () => {
  ...
  const abrirEditar = (alumno: Alumno) => {
    setAlumnoActual({ ...alumno, nombreCompleto: `${alumno.nombre || ''} ${alumno.apellido || ''}`.trim() });
    setEditando(true);
    setDialogOpen(true);
  };
  ...
  const guardar = () => {
    ...
    const operacion = editando && alumnoActual.id
      ? alumnoService.update(alumnoActual.id, alumnoAGuardar)
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.3 }}>
                      {alumno.telefono && (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8, color: '#475569' }}>
                          ...
                          <Typography variant="caption" sx={{ fontWeight: 500 }}>{alumno.telefono}</Typography>
                        </Box>
                      )}
                      {alumno.email && (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8, color: '#475569' }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, flexDirection: { xs: 'column', sm: 'row' } }}>
              ...
              <Box sx={{ flex: 1 }}>
                ...
                <TextField
                  ...
                  onChange={(e) => setAlumnoActual({ ...alumnoActual, estado: e.target.value as EstadoAlumno })}
                >
